<?php session_start();
if(isset($_SESSION['user']) && $_SESSION['rol'] == "administrador" && (isset($_REQUEST['modificarUser']) || isset($_GET['modificarUser'])))
{
?>
...
{
 $idu = explode("|",$_REQUEST['modificarUser'])[0];
}
...
$user = $clasUsuario->buscar_usuario_id($idu);
if($user != "vacio")
{
?>

<h2><font color="#178F99">Modificar usuario <?php echo $user->user ?></font></h2>
<table height="30"></table>

	<form name="frm_usuario" id="form" method="post" action="controladora_usuario.php" onsubmit="return prueba()" >
		<table  width="770" height="189" border="0">
			<caption align="bottom"></caption>

			<tr>
			<td width="200"><label for="nombre"><strong>Nombre y apellidos</strong></label></td>
			<td width="200"><label for="user"><strong>Usuario</strong></label></td>
			<td width="200"><label for="pass"><strong>Contraseña</strong></label></td>
			</tr>
			
			<tr>
			<td><input type="text" name="nombre" id="nomb" value="<?php echo $user->nombre ?>"/></td>
			<td><input type="text" name="usuario" id="user" value="<?php echo $user->user ?>"/></td>
			<td><input type="password" name="pass" id="contras" value="<?php echo $user->pass ?>"/></td>
			</tr>
			
			<tr height="30"></tr>
						
			<tr>
			<td><strong>Rol </strong><select name="rol" id="rol">
				...
				{
				?>
				<td><input name="activo" type="checkbox" value="si" checked /><strong>Activo</strong></td>			
				<?php  
				}
				else
				{
				?>
				<td><input name="activo" type="checkbox" value="si"/><strong>Activo</strong></td>
				<?php
				}
				?>
			</tr>
			...
			<tr>
			<td><input name="idU" type="hidden" value="<?php echo $idu ?>" /></td>
			<td colspan="2">
					<input type="submit" name="modificarUser" value="Modificar usuario" onclick=" if(confirm('Realmente desea modificar los datos del usuario'))  return true; else  return false;"/></td>
			</tr>
...
if(isset($_GET['modificarUser']))
{
   $var = explode("|",$_GET['modificarUser'])[1];
  if($var=='errorUserYaExis')
  {
         ?>
	     <script type="text/javascript" >
		 alert("Ya existe una persona con ese usuario en la BD")
		 </script>
		 <?php
  }
  if($var=='errorUserBD')
  {
         ?>
	     <script type="text/javascript" >
		 alert("Error al modificar el usuario en la Base Dato")
		 </script>
		 <?php
  } 
  if($var=='adicionUser')
